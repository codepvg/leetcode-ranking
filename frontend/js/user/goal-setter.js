...
function extractSolvedCount(data, metric) {
...
export function loadGoalSetter(username, data) {
  ...
  function showProgress(goal, solvedCount) {
    ...
    elCount.textContent = progress + " / " + goal.target + " solved this week";
  ...
  function render() {
    ...
    if (goal) {
      try {
        const solved = extractSolvedCount(data, goal.metric);
        ...
        elCount.textContent = "Data error, please refresh later";
        elDisplay.style.display = "block";
        elSetter.style.display = "none";
      }
    } else {
  ...
  if (elSaveBtn && !elSaveBtn.dataset.bound) {
    elSaveBtn.dataset.bound = "true";
    elSaveBtn.addEventListener("click", function () {
      ...
      const solved = extractSolvedCount(data, metric);
      const goal = saveGoal(username, metric, target);
      showProgress(goal, solved);
    });
  }

  if (elInput && !elInput.dataset.bound) {
    elInput.dataset.bound = "true";
  ...
  if (elReset && !elReset.dataset.bound) {
    elReset.dataset.bound = "true";
